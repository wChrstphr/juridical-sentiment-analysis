"""Scraper para baixar PDFs das sentenças dos processos do TJMS
...
def ler_numeros_processos(arquivo_csv="tjms/data/notebook1/numeros_processos.csv"):
...
async def baixar_pdf_processo(page, numero_processo, browser):
    ...
    try:
        ...
        await page.goto("https://esaj.tjms.jus.br/cpopg5/openMobile.do", wait_until="domcontentloaded")
        ...
        url_completa = "https://esaj.tjms.jus.br" + url_relativa
        ...
        pdf_url = "https://esaj.tjms.jus.br" + pdf_path
        
        # Baixar PDF
        os.makedirs("tjms/data/notebook1/decisoes", exist_ok=True)
        context = browser.contexts[0]
        response = await context.request.get(pdf_url)
        
        if response.ok:
            pdf_bytes = await response.body()
            filename = f"tjms/data/notebook1/decisoes/{numero_processo}.pdf"
...
async def executar_scraping():
    ...
    print("=" * 60)
    print("SCRAPER TJMS - Download de PDFs das Sentenças")
    print("=" * 60)
    ...
    print("=" * 60)
    print(f"Total de processos: {len(numeros_processos)}")
    print(f"Sucessos: {sucessos} ({sucessos*100//len(numeros_processos) if len(numeros_processos) else 0}%)")
    print(f"Falhas: {falhas} ({falhas*100//len(numeros_processos) if len(numeros_processos) else 0}%)")
    print("=" * 60)
